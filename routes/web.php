...
Route::get('master', function() {
    ...
    $totalQRCode = \App\Models\QRCodeItem::with([
        ...
        return dd(collect($mergedData));

// dd($mergedData)
    /*$d = \App\Models\LoginHistory::with([
                'retailer:id,name,mobile_number',
                'qRCodeItem:id,wd_id',
                'qRCodeItem.wd:id,city_id,code',
                'qRCodeItem.wd.city:id,name',
            ])
            ->select('id','q_r_code_item_id')
            ->get()
            ->groupBy(function ($item) {
                return $item['qRCodeItem']['wd']['code'];
            })
            ->map(function ($group, $code) {
                $totalRetailers = count($group);
                return [
                    'code' => $code,
    ...
    ->select('id','wd_id', 'is_redeemed') // Include the 'status' field in the select
    ->get()
    ->groupBy(function ($item) {
        return $item['wd']['code'];
    })
    ...
    ->select(
        ...
        \DB::raw('COUNT(DISTINCT login_histories.retailer_id) AS totalRetailers'),
        \DB::raw('SUM(q_r_code_items.is_redeemed) AS redeemed'),
        \DB::raw('COUNT(DISTINCT q_r_code_items.id) AS totalQRCode')
    )
    ->groupBy('q_r_code_items.wd_id', 'wd.code', 'cities.name')
    ->get()
    ->toArray();
